...
num_to_compress = min(5, len(linear_layers))  # Start with 5 layers to test accuracy
...
for i, (name, module) in enumerate(linear_layers[:num_to_compress]):
    ...
    # Simplified debug for first layer only
    if i == 0:
        print(f"\n[DEBUG] First layer '{name}': ratio={ratio:.2f}x, size={len(compressed)} bytes")
    ...
    # Verify scales look reasonable (first layer only)
    if i == 0:
        print(f"  Scales: shape={scales_to_store.shape}, range=[{scales_to_store.min():.6f}, {scales_to_store.max():.6f}]")
...
class CompressedLinear(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # DEBUG: Print only once to reduce noise
        if not hasattr(self, '_forward_called'):
            print(f"[FORWARD] CompressedLinear executing (will only print once per layer)")
