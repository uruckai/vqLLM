#!/usr/bin/env python3
"""
CORRECT SOLUTION: Keep compressed in RAM, decode on-demand with reusable buffer
- Weights stay COMPRESSED in system RAM
- Single reusable staging buffer for decompression
- Strict load→use→free per layer
- Sequentialized MLP to avoid concurrent weight allocations
"""
...
print("="*80)
print("ZSTD CORRECT SOLUTION - On-Demand Decompression")
print("="*80)
print("Compressed in RAM → Decode on demand → Use → Free immediately")
print("Single reusable buffer, no pre-decompression")
print()
...
for i, (name, module) in enumerate(linear_layers):
    ...
    compressed_weights[name] = {
        'compressed': compressed,  # Kept in RAM (compressed)
        'shape': weight.shape,
        'scale': scale,
        'dtype': weight.dtype
    }
...
# Create on-demand decompression layer
print("[3/5] Creating on-demand decompression model...")

class OnDemandLinear(nn.Module):
    """Decompress on EVERY forward pass, no caching"""
    ...
    def forward(self, x):
        # Decompress from RAM (via nvCOMP)
...
class SequentialMLP(nn.Module):
    """MLP that calls gate/up/down sequentially, not in one expression"""
    ...
    def forward(self, x):
        # Sequentialize to ensure only one weight is live at a time
...
for name, module in model.named_modules():
    # Replace MLP modules with sequentialized version
    if isinstance(module, LlamaMLP):
        # Get the three projections
        ...
        if gate_name in compressed_weights and up_name in compressed_weights and down_name in compressed_weights:
            ...
            # Replace entire MLP with sequentialized version
    ...
    # Replace other linear layers
    else:
        for child_name, child in list(module.named_children()):
            if isinstance(child, nn.Linear):
                full_name = f"{name}.{child_name}" if name else child_name
                
                # Skip if already handled as part of MLP
...
print()
print(f"Compressed size in RAM: {total_comp_size:.0f} MB")
print(f"Compression ratio: {(len(linear_layers)*10)/(total_comp_size/1024):.1f}x (approx)")
print()
